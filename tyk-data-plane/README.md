...
`tyk-data-plane` provides the default deployment of a Tyk data plane for Tyk Self Managed MDCB or Tyk Cloud users. 
...
You could find the list of version tags available from [Docker hub](https://hub.docker.com/u/tykio).
...
If you were using `tyk-hybrid` chart for existing release, you cannot upgrade directly. Please modify the `values-data-plane.yaml` base on your requirements and install using the new `tyk-data-plane` chart.
...
The Tyk Helm Chart can connect to `simple-redis` in the same namespace by default. You do not need to set Redis address and password in `values-data-plane.yamll`.

### Protect Confidential Fields with Kubernetes Secrets

In the `values-data-plane.yaml` file, some fields are considered confidential, such as `APISecret`, connection strings, etc.
