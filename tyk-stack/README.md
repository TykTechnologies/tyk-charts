...
helm repo update
helm show values tyk-helm/tyk-stack > values-single-dc.yaml --devel
```
...
Then just run:
```bash
helm install tyk-stack tyk-helm/tyk-stack -n tyk --create-namespace -f values-single-dc.yaml --devel
```
