...
Set `tyk-gateway.gateway.control.enabled` to true will allow you to run the [Gateway API](https://tyk.io/docs/tyk-gateway-api) on a separate port and protect it behind a firewall if needed. You can expose it over ingress if needed. Set `tyk-gateway.gateway.control.ingress.enabled` to true to make it possible. The ingress configuration is similar to main tyk ingress mentioned above.
